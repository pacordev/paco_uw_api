...
This is the detailed engineering reference for the Insurance Underwriting API — how it's
built, how to run it, and exactly how every endpoint and safeguard behaves. See the
[README](../README.md) for the high-level overview; this document goes deep on the parts a
contributor or integrator needs to actually work with the code.
...
4. [Viewing the API contract](#viewing-the-api-contract)
5. [Endpoints](#endpoints)
6. [Security](#security)
7. [Testing with Postman](#testing-with-postman)
8. [Notebook console](#notebook-console)
9. [Running the test suite](#running-the-test-suite)
10. [Dependency scanning](#dependency-scanning)
...
- Deploy target: **Vercel** (frontend) + **Render** (this API, as a persistent process, not
  serverless) + **Neon** (Postgres) — all three deployed, see the README's "Deployment"
  section. Render's process stays up, so we connect to Neon's *direct/unpooled* connection
  string rather than the PgBouncer-backed pooled one — a long-lived process doesn't need
  PgBouncer's connection-churn protection, and going direct sidesteps PgBouncer
  transaction-mode pooling fighting asyncpg's prepared statements.
...
Note: `POST /quotes` is rate-limited to 10/minute per IP (see "Security" above) — the
